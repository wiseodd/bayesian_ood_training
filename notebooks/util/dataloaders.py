...
def binary_MNIST(cls1, cls2, train=True, batch_size=None, augm_flag=True, val_size=500):
    if batch_size==None:
        if train:
            batch_size=train_batch_size
        else:
    ...
    dataset.targets = np.where(dataset.targets == cls1, 0., dataset.targets)
    dataset.targets = np.where(dataset.targets == cls2, 1., dataset.targets)
...
def Noise(dataset, train=True, batch_size=None, size=2000):
    ...
    dataset_val, dataset_test = data_utils.random_split(dataset, (size, rest))
...
def CIFAR10(train=True, batch_size=None, augm_flag=True, val_size=None):
    if batch_size==None:
        if train:
            batch_size=train_batch_size
        else:
...
def RotatedCIFAR10(train=True, batch_size=None, augm_flag=True, val_size=None, angle=60):
    if batch_size==None:
        if train:
            batch_size=train_batch_size
        else:
...
def binary_CIFAR10(cls1, cls2, train=True, batch_size=None, augm_flag=True, val_size=500):
    if batch_size==None:
        if train:
            batch_size=train_batch_size
        else:
...
def CIFAR100(train=False, batch_size=None, augm_flag=False, val_size=None):
    if batch_size==None:
        if train:
            batch_size=train_batch_size
        else:
